...
public class Gradebook {
	...
	public String calculateMedianLetter() { // Calculates the median letter grade.
		...
		//Collects every student's scores and adds them to an array.
		for (Student student: listOfStudents) {
			scores[i++] = student.getGrade().getScore();
		}

		// Sorting algorithm to organize it in ascending order.
		...
		// Checks the median score and transforms it to a letter.
		if (n % 2 == 0) {
	...
	public String replaceGrade(int pid, int newGrade)
	{
		for(Student student: listOfStudents)
		{
			if(student.getPid() == pid)
	...
	public void printAllStudentsLetter()
	{ //Prints all students with their letter grade.
		for (Student s: listOfStudents)
		{
